...
class Tracks:
    ...
    def _save_int(self, directory: Path):
        """Save the intensity image as a numpy array using np.save. In the future,
        could be changed to use zarr or other file types.

        Args:
            directory (Path): The directory in which to save the segmentation
        """
        out_path = directory / self.INT_FILE
        np.save(out_path, self.intensity_image)
    ...
    @classmethod
    def load(cls, directory: Path, seg_required=False, int_required=False) -> Tracks:
        ...
        Args:
            ...
            int_required (bool, optional): If true, raises a FileNotFoundError if the
                intensity image file is not present in the directory. Defaults to False.
        ...
        int_file = directory / cls.INT_FILE
        intensity_image = cls._load_intensity_img(int_file, int_required=int_required)

        attrs_file = directory / cls.ATTRS_FILE
        attrs = cls._load_attrs(attrs_file)

        return cls(graph, seg, intensity_image, **attrs)
    ...
    @staticmethod
    def _load_intensity_img(
        int_file: Path, int_required: bool = False
    ) -> np.ndarray | None:
        """Load intensity image from a file. If the file doesn't exist, either return
        None or raise a FileNotFoundError depending on the int_required flag.

        Args:
            int_file (Path): The npz file to load.
            int_required (bool, optional): If true, raise a FileNotFoundError if the
                file is not present. Defaults to False.

        Returns:
            np.ndarray | None: The intensity image array, or None if it wasn't present and
                seg_required was False.
        """
        ...
        if int_file.is_file():
            ...
            raise FileNotFoundError(f"No intensity image found at {int_file}")
        else:
            return None
    ...
    def _compute_node_attrs(
        ...
        Args:
            ...
            features (list[str], optional): features to be measured. If None are provided, all features in
            self.features will be measured.
        ...
        for node, time in zip(nodes, times, strict=False):
            ...
            if (
                ...
                and NodeAttr.INTENSITY_MEAN.value in features
            ):
            ...
            for feature in features:
                if feature == "intensity_mean" and self.intensity_image is None:
                    show_warning(
                        "Intensity image not provided, cannot compute intensity for this node!"
                    )
                    attrs[feature].append(None)
                else:
